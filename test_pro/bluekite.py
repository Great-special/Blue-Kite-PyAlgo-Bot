...
class BlueKiteModel:
    ...
    def predict_market_condition(self, data, encoded):
    ...
    def analyze_market(self):
        ...
        self.encoded = self.classify_market_condition(self.data)
        market_condition, probability = self.predict_market_condition(self.data, encoded=self.encoded)
        ...
        # Check for trend reversal and place orders with the market condition
        # Super Trend
        supertrend = self.calculate_supertrend()

        # Market analysis based on SuperTrend
        print(supertrend.iloc[-1] < self.data['Close'].iloc[-1], supertrend.iloc[-1],  self.data['Close'].iloc[-1])
        if supertrend.iloc[-1] < self.data['Close'].iloc[-1]: # supertrend.iloc[-1] < last_close
            ...
            if market_condition == 'Bullish' : # and probability > 70
                self.send_order(
                    symbol=self.symbol,
                    lot_size=self.lot_size,
                    order_type=BUY,
        ...
        if supertrend.iloc[-1] > self.data['Close'].iloc[-1]:
            ...
            if market_condition == 'Bearish': # and probability > 70
                self.send_order(
                    symbol=self.symbol,
                    lot_size=self.lot_size,
                    order_type=SELL,
...
def start_bot():
    ...
    model = BlueKiteModel(symbol='EURGBP', mode='live', time_frame=M15, retrain=True)
    model.analyze_market()
